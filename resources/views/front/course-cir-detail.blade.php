...
@section('content')
    ...
    <div class="ed_detail_head " style="background-color:#FFFBEB;" data-overlay="8">
        <div class="container">
    ...
    @endfor-->
                ...
                <div class="col-lg-5 col-md-5">
                    <div class="ed_view_price pl-4">
                        ...
                        <!-- <h2 class="theme-cl"> ₹{{ $course->price }}</h2> -->
					...
					// $daytime=json_decode($item->start_date . ' ' . $item->time);
					$cdattime = date('Y-m-d H:i:s', strtotime(($item->start_date . ' ' . $item->time)));
					$st_time = strtotime($cdattime) + 3600;

					// if (\Carbon\Carbon::now()->lte($item->start_date)) {
					// dd($item->start_date); exit;
					if ($st_time <= time()) {
					?>
                        <a href="https://meet.google.com/?pli=1" target="_blank" class="btn btn-block theme-bg enroll-btn"
                            style="color:#fff;">Click to join the class</a>
                        <?php } else {  ?>
                        <a href="https://meet.google.com/?pli=1" target="_blank" class="btn btn-block theme-bg enroll-btn"
                            style="color:#fff; display: none;">Join Class<i class="ti-angle-right"></i></a>
                        <?php }
					//} 
					?>
